...
function alpha_posted_on() {
	...
	$posted_on = sprintf(
		_x( 'Posted on %s', 'post date', 'alpha' ),
		'<a href="' . esc_url( get_permalink() ) . '" rel="bookmark">' . $time_string . '</a>'
	);

	$byline = sprintf(
		_x( 'by %s', 'post author', 'alpha' ),
...
function alpha_entry_footer() {
	...
	if ( 'post' == get_post_type() ) {
		...
		if ( $categories_list && alpha_categorized_blog() ) {
			printf( '<span class="cat-links">' . __( 'Posted in %1$s', 'alpha' ) . '</span>', $categories_list );
		}

		/* translators: used between list items, there is a space after the comma */
		$tags_list = get_the_tag_list( '', __( ', ', 'alpha' ) );
		if ( $tags_list ) {
			printf( '<span class="tags-links">' . __( 'Tagged %1$s', 'alpha' ) . '</span>', $tags_list );
		}
	}

	if ( ! is_single() && ! post_password_required() && ( comments_open() || get_comments_number() ) ) {
		echo '<span class="comments-link">';
		comments_popup_link( __( 'Leave a comment', 'alpha' ), __( '1 Comment', 'alpha' ), __( '% Comments', 'alpha' ) );
		echo '</span>';
	}

	edit_post_link( __( 'Edit', 'alpha' ), '<span class="edit-link">', '</span>' );
}
endif;
